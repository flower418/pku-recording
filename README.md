...
| `--workers N` | 分片并发数（默认 96，实测最优；网络差可调小到 32） |
...
**下载速度慢？**
回放存放在校内 CDN，速度受服务器负载影响。分片是逐个小文件，服务器对单连接有限速，
所以并发数越大越快（实测同一批分片：32 并发 8.5 MB/s → 64 并发 18.5 MB/s → 96 并发 22.5 MB/s，
超过 128 后收益很小）。网络差或想保守一点可以 `--workers 32`。
如果本机开了代理（Clash 等），可试 `pku-recording --no-proxy ...` 直连（实测差异不大，视线路而定）。
